...
OUTPUT_JSONL = os.path.join(CHECKPOINT_DIR, f"klix_articles_checkpoint_server_{SERVER_ID}.jsonl")
OUTPUT_CSV = os.path.join(CHECKPOINT_DIR, f"klix_articles_final_server_{SERVER_ID}.csv")
ERROR_LOG_CSV = os.path.join(CHECKPOINT_DIR, f"klix_errors_server_{SERVER_ID}.csv")
...
def extract_podnaslov(soup, news_json):
    """
    Podnaslov/lead.
    U HTML-u koji si poslao lead je u:
    span.lead
    """
...
def extract_article_text(soup):
    ...
    SADRZAJ uključuje i lead + glavni tekst.
    ...
    # Najvažniji selektor za novi Klix layout
    body_candidates = [
    ...
    if len(parts) <= 1:
        ...
        if article:
            ...
            # izbaci header dio da ne pokupi naslov/navigaciju/share
